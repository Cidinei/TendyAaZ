...
<div class="content">
                                ...
                                <div class="col-md-6 col-sm-12">
                                    <div class="form-group">
                                        <label class="control-label no-margin text-semibold mr-1">
                                            <strong>
                                                <h5 class="font-weight-bold">{{__('storeDashboard.ovCustomerDetails')}}</h5>
                                            </strong>
                                        </label>
                                        <br>
                                    ...
                                    <div class="form-group mb-1">
                                        <div class="d-flex justify-content-center align-items-center flex-column mb-1" style="border: 1px solid #ddd;">
                                            ...
                                            <div class="py-1 text-success @if ($order->orderstatus_id == 6) text-danger @endif" style="font-weight: 500;">
                                                ...
                                                @if ($order->orderstatus_id == 10) {{__('storeDashboard.opOrderStatus10')}} @endif
                                                @if ($order->orderstatus_id == 11) {{__('storeDashboard.opOrderStatus11')}} @endif
                                            </div>
                                        </div>
                                    </div>
 ...
 @endif
        ...
        <div class="col-md-4 mb-5">
            ...
            <div class="card">
                <div class="card-body">
                    <p class="text-center mb-0">
                        ...
                        <br>
                        <b>{{ __('orderScheduleLang.orderScheduleDateText') }}</b> {{ json_decode($order->schedule_date)->day }}, {{ json_decode($order->schedule_date)->date }}
                        <br>
                        <b>{{ __('orderScheduleLang.orderScheduleSlotText') }}:</b> {{ json_decode($order->schedule_slot)->open }} - {{ json_decode($order->schedule_slot)->close }}
                    </p>
                </div>
            </div>
            @endif
            ...
            <div class="card">
                <div class="card-body">
                    <p class="text-center mb-0">Cash Change Requested: {{ config('appSettings.currencyFormat') }}{{ $order->cash_change_amount }}</p>
                </div>
            </div>
            @endif
            @if($order->orderstatus_id == 3 || $order->orderstatus_id == 4 ||$order->orderstatus_id == 5 || $order->orderstatus_id == 6)
            @if($order->accept_delivery && $order->accept_delivery->user && $order->accept_delivery->user->name)
            <div class="card">
                <div class="card-body">
                    <p class="text-center mb-0"> <strong>{{ __('storeDashboard.opDeliveryBy') }}: {{ $order->accept_delivery->user->name }}</strong></p>
                </div>
            </div>
            @endif
            @endif
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <a href="{{ route('restaurant.confirmScheduledOrder', $order->id) }}"
                            class="mr-2 btn btn-lg confirmOrderBtn btn-success">  {{__('orderScheduleLang.dashboardConfirmScheduledOrder')}} <i
                ...
                <div class="card-body">
                    ...
                    <div class="d-flex justify-content-center">
                        ...
                        <a href="{{ route('restaurant.acceptOrder', $order->id) }}"
                            ...
                            class="icon-checkmark3 ml-1"></i></a>
                        <a href="{{ route('restaurant.cancelOrder', $order->id) }}"
                            class="mr-2 btn btn-lg cancelOrderBtn btn-danger" data-popup="tooltip" data-placement="top" title="{{__('storeDashboard.ovTooltipCancel')}}"> {{__('storeDashboard.ovCancel')}} <i
                            class="icon-cross ml-1"></i></a>
                        @endif

                        <!-- FoodomaProStoreDashBoardPro -->
                        @if (\Module::find("StoreDashBoardPro") && \Module::find("StoreDashBoardPro")->isEnabled()) 
                            @if($order->restaurant->is_change_status == 1)
                            <td>
                                @if($order->orderstatus_id == 2)
                                    @if($order->delivery_type == 1)
                                        <a href="{{ route('restaurant.markOrderSend', $order->id) }}"
                                            class="mr-2 btn btn-lg btn-warning"> {{__('storeDashboard.ovMarkAsSend')}} <i
                                            class="icon-checkmark3 ml-1"></i></a>
                                    @endif	
                                @endif
                            </td>
                            <td>
                                @if($order->orderstatus_id == 4 || $order->orderstatus_id == 7 )
                                <a href="{{ route('restaurant.markSelfPickupOrderAsCompleted', $order->id) }}"
                                    class="mr-2 btn btn-lg btn-success"> {{__('storeDashboard.ovMarkAsCompleted')}} <i
                                    class="icon-checkmark3 ml-1"></i></a>
                                @endif
                            </td>
                            @endif
                        ...
                        <a href="{{ route('restaurant.markOrderReady', $order->id) }}"
                            class="mr-2 btn btn-lg btn-warning"> {{__('storeDashboard.ovMarkAsReady')}} <i
                            class="icon-checkmark3 ml-1"></i></a>
                        @endif
                        ...
                        @if (\Module::find("ThermalPrinterPro") && \Module::find("ThermalPrinterPro")->isEnabled()) 
                            <a href="{{ route('restaurant.printOrder', $order->unique_order_id) }}"
                            class="mr-2 btn btn-lg btn-danger"> {{ Lang::get('thermalprinterpro::default.thermalPrinterOrder') }} <i
                            class="icon-printer ml-1"></i></a>                 
                        @endif 
