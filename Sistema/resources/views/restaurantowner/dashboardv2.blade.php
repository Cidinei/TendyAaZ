...
@if (\Module::find("StoreDashBoardPro") && \Module::find("StoreDashBoardPro")->isEnabled())
    <div class="content">
                ...
                <div class="card-body px-2" style="height: 95vh; overflow-y: scroll;">
                    ...
                    <div class="single-order-parent p-2 mb-3">
                        <div class="single-order single-new-order" data-order="{{ $nO->unique_order_id }}">
                            <a href="javascript:void(0)" class="letter-icon-title"><b>#{{ substr ($nO->unique_order_id, -9)  }}</b></a>
                            ...
                            <p class="my-1 small">
                                <mark class="px-0">
                                {{ json_decode($nO->schedule_date)->day }}, {{ json_decode($nO->schedule_date)->date }} 
                        ...
                        <div class="new-order-actions d-flex mt-1">
                            ...
                            @if($nO->orderstatus_id == 10)
                                <a href="{{ route('restaurant.confirmScheduledOrder', $nO->id) }}"
                                    class="btn btn-success mr-1 confirmOrderBtnTableList" data-id={{ $nO->id }}> {{__('orderScheduleLang.dashboardConfirmScheduledOrder')}} </a>
                            @endif
                ...
                <div class="card-body px-2" style="height: 95vh; overflow-y: scroll;">
                    ...
                    <div class="single-order-parent p-2 mb-3">
                        <div class="single-order" data-order="{{ $pO->unique_order_id }}">
                            <a href="javascript:void(0)" class="letter-icon-title"><b>#{{ substr ($pO->unique_order_id, -9)  }}</b></a>
                            ...
                            <p class="my-1 small">
                                <mark class="px-0">
                                {{ json_decode($pO->schedule_date)->day }}, {{ json_decode($pO->schedule_date)->date }} 
                        ...
                        <p class="liveTimer my-1 text-center min-fit-content order-dashboard-time" title="{{ $pO->created_at }}"><i class="icon-spinner10 spinner position-left"></i></p>
                            ...
                            @if($pO->orderstatus_id == 2 || $pO->orderstatus_id == 3)
                                ...
                                <a href="{{ route('restaurant.markOrderReady', $pO->id) }}"
                                    class="btn btn-warning actionAfterAccept mr-1">
                                ...
                                @if($pO->restaurant->is_change_status == 1)
                                    @if($pO->delivery_type == 1)
                                        <a href="{{ route('restaurant.markOrderSend', $pO->id) }}"
                                            class="btn btn-info actionAfterAccept mr-1">
                                            {{ Lang::get('storedashboardpro::default.ovMarkAsSend') }} </a>
                                    @endif
                                @endif
            
                                <a href="{{ route('restaurant.printOrder', $pO->unique_order_id) }}"
                                class="btn btn-danger actionAfterAccept mr-1">
                                {{ Lang::get('thermalprinterpro::default.thermalPrinterOrder') }} </a>
        ...
        <!-- Change Josimar Brasil -->
        @if($agent->isDesktop()) 
            @if(count($selfpickupOrders) > 0) @endif
            <div class="col @if($agent->isMobile()) d-inline-block @endif">
                <div class="card">
                    <div class="text-center">
                        <h4 class="mt-2 mb-0"><strong>{{ Lang::get('storedashboardpro::default.dashboardOrdersOnRoute') }}<span class="badge badge-flat dark-badge ml-1">
                            {{ count($onRouteOrReadyForPickupOrders) }}
                            </span></strong>
                        </h4>
                        <hr class="mt-1 mb-0">
                    </div>
                    <div class="card-body px-2" style="height: 95vh; overflow-y: scroll;">
                        ...
                        @endif
                        @foreach($onRouteOrReadyForPickupOrders as $spO)
                        <div class="single-order-parent p-2 mb-3">
                            <div class="single-order single-self-pickup-order" data-order="{{ $spO->unique_order_id }}" data-orderstatusid="{{ $spO->orderstatus_id }}">
                                <a href="javascript:void(0)" class="letter-icon-title"><b>#{{ substr ($spO->unique_order_id, -9)  }}</b></a>
                                <br>
                                {{ $spO->restaurant->name }}
                                <br>
                            ...
                            @if($spO->restaurant->is_change_status == 1)
                                @if($spO->orderstatus_id == 4)
                                <a href="{{ route('restaurant.markSelfPickupOrderAsCompleted', $spO->id) }}"
                                    class="btn btn-success mr-1 actionAfterAccept">
                                    {{ Lang::get('storedashboardpro::default.dashboardMarkAsCompletedDelivery') }} </a>
                                @endif
                            @endif
            
                            @if($spO->orderstatus_id == 7 )
                            <a href="{{ route('restaurant.markSelfPickupOrderAsCompleted', $spO->id) }}"
                                class="btn btn-primary mr-1 actionAfterAccept">
                                {{ Lang::get('storedashboardpro::default.dashboardMarkAsCompletedRetired') }} </a>
                            @endif
                ...
                <div class="card-body px-2" style="height: 95vh; overflow-y: scroll;">
                    ...
                    <div class="single-order-parent p-2 mb-3">
                        <div class="single-order" data-order="{{ $ogO->unique_order_id }}">
                            <a href="javascript:void(0)" class="letter-icon-title"><b>#{{ substr ($ogO->unique_order_id, -9)  }}</b></a>
                            <br>
                            {{ $ogO->restaurant->name }}
                            <br>
                            @php
