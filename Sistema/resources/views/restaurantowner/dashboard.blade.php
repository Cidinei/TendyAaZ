...
@if (\Module::find("StoreDashBoardPro") && \Module::find("StoreDashBoardPro")->isEnabled()) 
    <div class="content mb-5">
        ...
        <div class="row pt-4 p-0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        ...
                                        <small>
                                            <mark class="px-0">
                                            {{ json_decode($nO->schedule_date)->day }}, {{ json_decode($nO->schedule_date)->date }}<br>
                                            {{ json_decode($nO->schedule_slot)->open }} - {{ json_decode($nO->schedule_slot)->close }}
                                            </mark>
                                        </small>
                                        @endif
                                    </td>
                                    <td class="text-center new-order-actions">
                                        ...
                                        <a href="javascript:void(0)"
                                            class="mr-2 btn btn-primary accpetOrderBtnTableList" data-id={{ $nO->id }}><i
                                            class="icon-checkmark3 ml-1"></i> {{__('storeDashboard.dashboardAcceptOrder')}} </a>
                                        @endif
                                        @if($nO->orderstatus_id == 10)
                                            <a href="{{ route('restaurant.confirmScheduledOrder', $nO->id) }}"
                                                class="mr-2 btn btn-success confirmOrderBtnTableList" data-id={{ $nO->id }}><i
                                                class="icon-checkmark3 ml-1"></i> {{__('orderScheduleLang.dashboardConfirmScheduledOrder')}} </a>
                                        @endif

                                        <a href="{{ route('restaurant.cancelOrder', $nO->id) }}"
                                            class="mr-2 btn btn-danger cancelOrderBtnTableList" data-popup="tooltip"
                                            data-placement="right" title="{{ __('storeDashboard.dashboardDoubleClickMsg') }}"><i
                                            class="icon-cross ml-1"></i> {{__('storeDashboard.dashboardCancelOrder')}} </a>
                                    </td>
                                    <td>
                                        {{ $nO->restaurant->name }}
                                    </td>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        ...
                                        <small>
                                            <mark class="px-0">
                                            {{ json_decode($pO->schedule_date)->day }}, {{ json_decode($pO->schedule_date)->date }}<br>
                                            {{ json_decode($pO->schedule_slot)->open }} - {{ json_decode($pO->schedule_slot)->close }}
                                            </mark>
                                        </small>
                                        @endif
                                    </td>
                                    <td class="text-center accepted-order-actions">
                                        ...
                                        <a href="{{ route('restaurant.markOrderReady', $pO->id) }}"
                                            class="mr-2 btn btn-warning actionAfterAccept"><i
                                            class="icon-checkmark3 ml-1"></i> {{__('storeDashboard.dashboardMarkAsReady')}} </a>
                                        @endif
                                        @if($pO->delivery_type == 2 && $pO->orderstatus_id == 7)
                                        <a href="{{ route('restaurant.markSelfPickupOrderAsCompleted', $pO->id) }}"
                                            class="mr-2 btn btn-success actionAfterAccept"><i
                                            class="icon-checkmark3 ml-1"></i> {{__('storeDashboard.dashboardMarkAsCompleted')}} </a>
                                        @endif
                                        <!-- @author Josimar-Brasil  -->
                                        @if($pO->orderstatus_id == 2 || $pO->orderstatus_id == 3)
                                            @if($pO->restaurant->is_change_status == 1)
                                                @if($pO->delivery_type == 1)
                                                    <a href="{{ route('restaurant.markOrderSend', $pO->id) }}"
                                                        class="mr-2 btn btn-info"> <i
                                                        class="icon-checkmark3 ml-1"></i> {{ Lang::get('storedashboardpro::default.ovMarkAsSend') }} </a>
                                                @endif
                                            @endif

                                            <!-- FoodomaProThermalPrinterPro -->
                                            @if (\Module::find("ThermalPrinterPro") && \Module::find("ThermalPrinterPro")->isEnabled()) 
                                                <a href="{{ route('restaurant.printOrder', $pO->unique_order_id) }}"
                                                class="mr-2 btn btn-lg btn-danger"> <i
                                                class="icon-printer ml-1"></i> {{ Lang::get('thermalprinterpro::default.thermalPrinterOrder') }} </a>                
                                            @endif 
                                            <!-- endFoodomaProThermalPrinterPro -->
                                        @endif
            ...
            <!-- @author Josimar-Brasil - Creates the orders en route or ready for pickup panel -->
            <div class="col-xl-12">
                <div class="panel panel-flat dashboard-main-col mt-4">
                <div class="panel-heading">
                    <h4 class="panel-title pl-3 pt-3"><strong>{{ Lang::get('storedashboardpro::default.dashboardOrdersOnRoute') }}</strong></h4>
                    
                </div>
                @if(count($onRouteOrReadyForPickupOrders))
                <div class="table-responsive">
                    <table class="table text-nowrap">
                    <thead class="thead-dark">
                        <tr>
                        ...
                        <th>{{__('storeDashboard.dashboardStore')}}</th>
                        <th>{{__('storeDashboard.dashboardPrice')}}</th>
                        <th>{{ Lang::get('storedashboardpro::default.dashboardLastUpdateTime') }}</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($onRouteOrReadyForPickupOrders as $aO)
                        <tr>
                            <td>
                                <a href="{{ route('restaurant.viewOrder', $aO->unique_order_id) }}"
                                    class="letter-icon-title">#{{ substr ($aO->unique_order_id,-9) }}
                                </a>
                                @if($aO->schedule_slot != null)
                                    <br>
                                    <small>
                                        <mark class="px-0">
                                        {{ json_decode($aO->schedule_date)->day }}, {{ json_decode($aO->schedule_date)->date }}<br>
                                        {{ json_decode($aO->schedule_slot)->open }} - {{ json_decode($aO->schedule_slot)->close }}
                                        </mark>
                                    </small>
                                @endif
                            ...
                            <td class="text-center accepted-order-actions">
                                @if($aO->restaurant->is_change_status == 1)
                                    @if($aO->orderstatus_id == 4)
                                        <a href="{{ route('restaurant.markSelfPickupOrderAsCompleted', $aO->id) }}"
                                            class="mr-2 btn btn-success actionAfterAccept"><i
                                            class="icon-checkmark3 ml-1"></i></b> {{ Lang::get('storedashboardpro::default.dashboardMarkAsCompletedDelivery') }} 
                                        </a> 
                                    @endif
                                @endif

                                @if($aO->orderstatus_id == 7 )
                                    <a href="{{ route('restaurant.markSelfPickupOrderAsCompleted', $aO->id) }}"
                                        class="mr-2 btn btn-primary actionAfterAccept"><i
                                        class="icon-checkmark3 ml-1"></i> {{ Lang::get('storedashboardpro::default.dashboardMarkAsCompletedRetired') }} 
                                    </a> 
                                @endif
                            </td>
                            <td>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center new-order-actions">
                                        ...
                                        @if($nO->orderstatus_id == 10)
                                            <a href="{{ route('restaurant.confirmScheduledOrder', $nO->id) }}"
                                                class="btn btn-success btn-labeled btn-labeled-left mr-2 confirmOrderBtnTableList" data-id={{ $nO->id }}> <b><i
                                                class="icon-checkmark3 ml-1"></i> </b> {{__('orderScheduleLang.dashboardConfirmScheduledOrder')}} </a>
                                        @endif
...
@endif 
<!-- endFoodomaProStoreDashBoardPro -->
