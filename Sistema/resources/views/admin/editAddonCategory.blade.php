@extends('admin.layouts.master')
@section("title") Editar categoria de adicionais - Dashboard
@endsection
@section('content')
<div class="page-header">
    <div class="page-header-content header-elements-md-inline">
        <div class="page-title d-flex">
            <h4><i class="icon-circle-right2 mr-2"></i>
                <span class="font-weight-bold mr-2">Editando</span>
                <span class="badge badge-primary badge-pill animated flipInX">"{{ $addonCategory->name }}"</span>
            </h4>
            <a href="#" class="header-elements-toggle text-default d-md-none"><i class="icon-more"></i></a>
        </div>
    </div>
</div>
<div class="content">
    <div class="col-md-8">
        <div class="card">
            <div class="card-body">
                <form action="{{ route('admin.updateAddonCategory') }}" method="POST" enctype="multipart/form-data">
                    <legend class="font-weight-semibold text-uppercase font-size-sm">
                        <i class="icon-tree6 mr-2"></i> Categoria de adicional
                    </legend>
                    <input type="hidden" name="id" value="{{ $addonCategory->id }}">
                    <div class="form-group row">
                        <label class="col-lg-3 col-form-label"><span class="text-danger">*</span>Nome:</label>
                        <div class="col-lg-9">
                            <input value="{{ $addonCategory->name }}" type="text" class="form-control form-control-lg" name="name"
                                placeholder="Nome da categoria de adicional" required>
                        </div>
                    </div>
                    <div class="form-group row">
                        <label class="col-lg-3 col-form-label"><span class="text-danger">*</span>Type:</label>
                        <div class="col-lg-9">
                            <select name="type" class="form-control form-control-lg select">
                                <option value="SINGLE" @if($addonCategory->type == "SINGLE") selected="selected" @endif> Seleção única </option>
                                <option value="MULTI" @if($addonCategory->type == "MULTI") selected="selected" @endif> Seleção múltipla </option>
                            </select>
                        </div>
                    </div>

                    <!-- Módulo de complementos -->
                    @if ( \Module::find("AddonsPro")->isEnabled() == false)

                        <div class="form-group row @if($addonCategory->type == "SINGLE") hidden @endif" id="addonLimitInputBlock">
                            <label class="col-lg-3 col-form-label">Limite <i class="icon-question3 ml-1" data-popup="tooltip" title="Número máximo de adicionais que o cliente poderá selecionar" data-placement="top"></i></label>
                            <div class="col-lg-9">
                            <input value="{{ $addonCategory->addon_limit }}" type="text" class="form-control form-control-lg" name="addon_limit" placeholder="Limite de adicionais" value="0">
                            <span class="small">Deixe 0 para deixar sem limites.</span>
                            </div>
                        </div>
                        <script>
                            $("[name='type']").change(function() {
                                let addonType = $(this).val();
                                if (addonType == "MULTI") {
                                    $('#addonLimitInputBlock').removeClass('hidden');
                                } else {
                                    $('#addonLimitInputBlock').addClass('hidden');
                                }
                            });
                        </script>
                        
                    @endif 
                    <!-- Módulos de complementos END -->

                    <div class="form-group row">
                        <label class="col-lg-3 col-form-label">Descrição:</label>
                        <div class="col-lg-9">
                            <input type="text" class="form-control form-control-lg" name="description" value="{{ $addonCategory->description }}" 
                                placeholder="Curta descrição (50-80 characters)" >
                                <span class="help-text text-muted"> Importante adicionar descrição para conseguir indentificar a categoria</span>
                        </div>
                    </div>

                    <div class="mt-4">
                        <legend class="font-weight-semibold text-uppercase font-size-sm" id="addonsLegend">
                            <i class="icon-list2 mr-2"></i> Adicionais
                        </legend>
                    </div>
                    @foreach ($addons as $addon)
                    <div class='form-group row'> 
                        <div class='col-lg-5'>
                        <input type="text" hidden name="addon_old[{{$loop->index}}][id]" value="{{$addon->id}}">
                            <input type='text' class='form-control clock form-control-lg'  value="{{ $addon->name }}" name="addon_old[{{$loop->index}}][name]" placeholder="Nome do adicional"> 
                        </div> 
                        <div  class='col-lg-5'>
                            <input type='text' class='form-control clock form-control-lg' value="{{ $addon->price }}" name="addon_old[{{$loop->index}}][price]" placeholder="Valor do adicional"> 
                        </div> 
                        <div class='col-lg-1'> 
                            <div class="checkbox checkbox-switchery ml-1" style="padding-top: 0.8rem;">
                                <label>
                                <input value="true" type="checkbox" class="action-switch"
                                @if($addon->is_active) checked="checked" @endif data-id="{{ $addon->id }}">
                                </label>
                            </div>
                        </div>
                        <div class='col-lg-1'> 
                        <div class='btn btn-danger' data-popup='tooltip' data-placement='right' onclick="del({{$addon->id}})" title='Deletar Adicional'><i class='icon-bin'></i></div>
                        
                    </div>
                        
                    </div>
                    @endforeach
                    <div id="addon" class="timeSlots">
                    </div>
                    <a href="javascript:void(0)" onclick="add(this)" class="btn btn-secondary btn-labeled btn-labeled-left mr-2"> <b><i class="icon-plus22"></i></b>Add Adicional</a>

                    <!-- Módulo de complementos -->
                    @if (\Module::find("AddonsPro") && \Module::find("AddonsPro")->isEnabled()) 
                        <div id="_min_and_max" class="mt-4" style="display: none">
                            <div class="form-group row">
                                <label class="col-lg-3 col-form-label">Mínimo:</label>
                                <div class="col-lg-9">
                                    <input type="number" class="form-control form-control-lg" name="minimum_qty" placeholder="Seleção mínima" value="<?= $addonCategory->minimum_qty ?>"  min="0" max="999">
                                </div>
                            </div>
                            <div class="form-group row">
                                <label class="col-lg-3 col-form-label">Maximum Qty:</label>
                                <div class="col-lg-9">
                                    <input type="number" class="form-control form-control-lg" name="maximum_qty" placeholder="Seleção máxima" value="<?=$addonCategory->maximum_qty?>" min="0" max="999">
                                </div>
                            </div>
                        </div>
                        <div id="_required_option" class="checkbox checkbox-switchery ml-1 row" style="padding-top: 0.8rem;">
                            <label class="col-lg-3 col-form-label">Seleção obrigatória:</label>
                            <label class="col-lg-9">
                                <input value="true" name="add_required" type="checkbox" class="action-switch2" 
                                 @if($addonCategory->add_required === 1) checked="checked" @endif data-id="{{ $addonCategory->id }}">
                            </label>
                        </div>
                    @endif 
                    <!-- Módulo de complementos END -->
                    
                    
                    @csrf
                    <div class="text-right">
                        <button type="submit" class="btn btn-primary">
                        ATUALZIAR
                        <i class="icon-database-insert ml-1"></i>
                        </button>
                    </div>
                </form>
            </div>
        </div>
    </div>
</div>

<!-- Módulo de complementos -->
@if (\Module::find("AddonsPro") && \Module::find("AddonsPro")->isEnabled()) 
    @php include(base_path('Modules/AddonsPro/includes/javascript/edit-addon-category.php')) @endphp    
@endif 
<!-- Módulo de complementos END -->

<script>
        
        function add(data) {
            $('#addonsLegend').removeClass('hidden');
            var newAddon = document.createElement("div");
            newAddon.innerHTML ="<div class='form-group row'> <div class='col-lg-5'><input type='text' class='form-control  form-control-lg' placeholder='Addon Name' name='addon_names[]' required> </div> <div class='col-lg-5'> <input type='text' class='form-control  form-control-lg' name='addon_prices[]' placeholder='Addon Price'  required> </div> <div class='col-lg-2'><button class='remove btn btn-danger' data-popup='tooltip' data-placement='right' title='Remove Addon'><i class='icon-cross2'></i></button></div></div>";
            document.getElementById('addon').appendChild(newAddon);
        }

        function del(id) {
          var r = confirm("Confirm Delete Addon");
          if (r == true) {
            let url = "{{ url('admin/addon/delete/') }}/"+id;
            window.location.href = url;
          }
        }

        $(function() {

         $('.price').numeric({allowThouSep:false, maxDecimalPlaces: 2 });

         //Switch Action Function
         if (Array.prototype.forEach) {
                var elems = Array.prototype.slice.call(document.querySelectorAll('.action-switch'));
                elems.forEach(function(html) {
                    var switchery = new Switchery(html, { color: '#8360c3' });
                });

                /*Módulo de complementos*/ 
                var elems = Array.prototype.slice.call(document.querySelectorAll('.action-switch2'));
                elems.forEach(function(html) {
                    var switchery = new Switchery(html, { color: '#8360c3' });
                });
                /* Módulo de complementos END*/
            }
            else {
                var elems = document.querySelectorAll('.action-switch');
                for (var i = 0; i < elems.length; i++) {
                    var switchery = new Switchery(elems[i], { color: '#8360c3' });
                }

                /*Módulo de complementos*/ 
                var elems = document.querySelectorAll('.action-switch2');
                for (var i = 0; i < elems.length; i++) {
                    var switchery = new Switchery(elems[i], { color: '#8360c3' });
                }
                /* Módulo de complementos END*/
            }

          $('.action-switch').click(function(event) {
             let id = $(this).attr("data-id")
             let url = "{{ url('/admin/addon/disable/') }}/"+id;
             window.location.href = url;
          });

            /* Módulo de complementos */ 
            $('.action-switch2').click(function(event) {
                let id = $(this).attr("data-id")
                let url = "{{ url('/addonspro/addoncategory/disablerequired/') }}/"+id;
                window.location.href = url;
            });
            /* Módulo de complementos END */

      
          $('.select').select2({
              minimumResultsForSearch: -1,
          });

          $(document).on("click", ".remove", function() {
              $(this).tooltip('hide')
              $(this).parent().parent().remove();
          });
      }); 
</script>
@endsection