        ...
        <div class="card">
            <div class="card-body">
                <legend class="font-weight-semibold text-uppercase font-size-sm">
                    <i class="icon-address-book mr-2"></i> Lojas atribuídas
                </legend>
                <div class="form-group row form-group-feedback form-group-feedback-right">
                    @if(count($userRestaurants) === 0)
                    <div class="col-lg-9">
                        <p class="text-warning">{{ $user->name }} ainda não foi designado para receber entregas de nenhuma loja.</p>
                    </div>
                    @else
                    <br>
                    <div class="col-lg-9">
                        <p><strong>{{ $user->name }}</strong> Atribuído a <strong>{{ $userRestaurants->count() }} </strong> Lojas.</p>
                ...
                @if(!$user->delivery_guy_detail)
                    <p>Todos os detalhes para {{ $user->name }} não são preenchidos. Por favor, preencha todos os detalhes obrigatórios para atribuir lojas a {{ $user->name }}</p>
                    <strong><a href="{{ route('admin.get.editUser', $user->id) }}#deliveryGuyDetails">Click Here </a></strong> Editar detalhes
                @else
    ...
    <div class="col-md-12 hidden" id="manageRestaurantsBlock">
        <div class="form-group form-group-feedback form-group-feedback-right search-box">
            <input type="text" class="form-control form-control-lg search-input"
                placeholder="Filtre por nome da loja...">
                ...
                <div class="col-lg-12">
                    <form action="{{ route('admin.updateDeliveryGuysRestaurants') }}" method="POST">
                        ...
                        <div class="text-right mb-4">
                            <button type="button" class="btn btn-primary btn-labeled btn-labeled-left btn-sm" id="checkAll" data-popup="tooltip" title="Duplo clique para marcar todas" data-placement="left">
