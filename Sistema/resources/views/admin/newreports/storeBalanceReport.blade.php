...
@section("title") Saldo da loja - Relatório
@endsection
@section('content')
...
<div class="page-header-content header-elements-md-inline">
  <div class="page-title d-flex">
    <h4><i class="icon-circle-right2 mr-2"></i>
      <span class="font-weight-bold mr-2">Relatório de saldo da loja</span>
                ...
                <table class="table">
                    <thead>
                        <tr>
                          <th>ID</th>
                          <th>Nome da loja</th>
                          <th>Saldo de vendas</th>
                          <th>Saldo a pagar</th>
                          <th>Pagamento solicitado</th>
                          <th>Balanço total</th>
                          <th>Data e hora do último pagamento solicitado</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($restaurants as $key => $restaurant)
                          ...
                          <tr>
                            ...
                            <td>{{ config('appSettings.currencyFormat') }}{{ number_format($payoutBalance, 2) }}</td>
                            <td>{{ config('appSettings.currencyFormat') }}{{ number_format($payoutBalanceAfterCommission, 2) }}</td>
                            <td>{{ config('appSettings.currencyFormat') }}{{ number_format($requestedBalanceAfterCommission, 2) }}</td>
                            <td>{{ config('appSettings.currencyFormat') }}{{ number_format($totalBalance,2) }}</td>
                    ...
                    <tfoot>
                       <tr>
                            <th>TOTAL</th>
                            <th></th>
                            <th>{{ config('appSettings.currencyFormat') }}{{ number_format($grandTotalSalesBalance, 2) }}</th>
                            <th>{{ config('appSettings.currencyFormat') }}{{ number_format($grandTotalPayableBalance,2) }}</th>
                            <th>{{ config('appSettings.currencyFormat') }}{{ number_format($grandTotalPayoutBalance, 2) }}</th>
                            <th>{{ config('appSettings.currencyFormat') }}{{ number_format($grandTotalBalance,2) }}</th>
                            <th></th>
                       </tr>
                    </tfoot>
                </table>
