...
@section("title") Pagamento de Pedidos - Relatório
@endsection
@section('content')
<style>
    .range-selector {
    ...
    <div class="page-header-content header-elements-md-inline">
        <div class="page-title d-flex">
            <h4><i class="icon-circle-right2 mr-2"></i>
                <span class="font-weight-bold mr-2">Relatório de pagamentos de pedidos</span>
        ...
        <div class="row">
            <div class="col-md-8 mt-4">
                <a href="javascript:void(0)" id="printButton" class="btn btn-sm btn-primary my-2"
                    style="color: #fff; border: 1px solid #ccc; float: right;"><i class="icon-printer mr-1"></i> Imprimir Relatório</a>
                        ...
                        <tbody>
                            ...
                            @foreach ($orders as $order)
                                ...
                                <tr>
                                    ...
                                    <td>{{ config('appSettings.currencyFormat') }}{{ $orderAmount }}</td>
                                    <td>{{ config('appSettings.currencyFormat') }}{{ $orderWalletAmount }}</td>
                                    <td>{{ config('appSettings.currencyFormat') }}{{ $orderPayment }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                        <tfoot>
                            ...
                            <th></th>
                            <th></th>
                            <th>{{ config('appSettings.currencyFormat') }}{{ $orderAmountTotal }}</th>
                            <th>{{ config('appSettings.currencyFormat') }}{{ $orderWalletTotal }}</th>
                            <th>{{ config('appSettings.currencyFormat') }}{{ $orderPaymentTotal }}</th>
                        </tfoot>
                    </table>
                </div>
            </div>
