...
class Restaurant extends Model implements Sortable
{
    ...
    public function getPayoutBalanceAttribute() {
        $balance = RestaurantEarning::where('restaurant_id', $this->id)->where('is_requested', 0)->latest()->first();
        return $balance;
    }
    
    public function getRequestedBalanceAttribute() {
        $balance = RestaurantEarning::where('restaurant_id', $this->id)->where('is_requested', 1)->where('is_processed', 0)->latest()->first();
        return $balance;
    }
