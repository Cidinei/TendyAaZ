...
class DeliveryController extends Controller
{
    ...
    public function getDeliveryOrders(Request $request)
    {
        ...
        foreach ($orders as $order) {
            ...
            foreach ($userRestaurants as $ur) {
                ...
                if ($order->restaurant->id == $ur->id && $is_in_limit) {

                    /*FoodomaProDeliveryRadiusPro*/ 
                    if (\Module::find("DeliveryRadiusPro") && \Module::find("DeliveryRadiusPro")->isEnabled()) { 
                        if(isset($deliveryUser->delivery_guy_detail->delivery_radius) && !empty($deliveryUser->delivery_guy_detail->delivery_radius) && !empty($lat) && !empty($lng)) {
                            if(\Modules\DeliveryRadiusPro\Http\Controllers\DeliveryRadiusProController::checkRadiusDelivery($lat, $lng, $order->restaurant, $deliveryUser->delivery_guy_detail->delivery_radius)) {
     ...
     * @param $order
     */
    public function canPerformAction($deliveryGuy, $order)
    {
        //if order cancelled then cannot perform action...
