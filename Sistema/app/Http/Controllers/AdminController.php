...
class AdminController extends Controller
{
    ...
    public function saveNewUser(Request $request)
    {
        try {
            ...
            if ($user->hasRole('Delivery Guy')) {
                ...
                /*FoodomaProDeliveryRadiusPro*/ 
                if (\Module::find("DeliveryRadiusPro") && \Module::find("DeliveryRadiusPro")->isEnabled()) { 
                    if(isset($request->delivery_radius)) {
                        $deliveryGuyDetails->delivery_radius = $request->delivery_radius;
                    }
                } 
                /*endFoodomaProDeliveryRadiusPro*/
    ...
    public function viewOrder($order_id)
    {
        ...
        if ($order) {
            $activities = Activity::where('subject_id', $order->id)->get();
    ...
    public function saveNewRestaurant(Request $request)
    {
        ...
        if (\Module::find("StoreDashBoardPro") && \Module::find("StoreDashBoardPro")->isEnabled()) { 
            ...
            if ($request->is_change_status == 'true') {
                $restaurant->is_change_status = true;
            } else {
                $restaurant->is_change_status = false;
            }
        } 
        ...
        try {
            if ($order->orderstatus_id != 5 || $order->orderstatus_id != 6) {
                ...
                activity()
                    ->performedOn($order)
                    ->causedBy(Auth::user())
                    ->withProperties(['type' => 'cancel'])->log('Order cancelled by Admin');
    ...
    public function approvePaymentOfOrder($order_id)
    {
        ...
        if ($order->orderstatus_id == '8') {
            ...
            if (\Module::find("NotifyStoreAndDeliveryGuy") && \Module::find("NotifyStoreAndDeliveryGuy")->isEnabled()) { 
                \Modules\NotifyStoreAndDeliveryGuy\Traits\PushNotifyTrait::oneSignalNotification('Store Owner',$order->restaurant_id);
            } 
