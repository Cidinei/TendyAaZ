...
class RestaurantOwnerController extends Controller
{
    public function dashboard()
    {
        ...
        $onRouteOrReadyForPickupOrders = Order::whereIn('restaurant_id', $restaurantIds)
        ->whereIn('orderstatus_id', ['4', '7'])
        ->orderBy('id', 'DESC')
        ->get();
        ...
        $arrayData = [
            ...
            /*FoodomaProStoreDashBoardPro*/ 	    
            'onRouteOrReadyForPickupOrders' => $onRouteOrReadyForPickupOrders,
            /*endFoodomaProStoreDashBoardPro*/ 
        ];
    ...
    public function acceptOrder($id)
    {
        ...
        if ($order->orderstatus_id == '1') {
            ...
            if (\Module::find("NotifyStoreAndDeliveryGuy") && \Module::find("NotifyStoreAndDeliveryGuy")->isEnabled()) { 
                \Modules\NotifyStoreAndDeliveryGuy\Traits\PushNotifyTrait::oneSignalNotification('Delivery Guy',$order->restaurant_id); 
            } 
                    ...
                    if ($restaurant) {
                        ...
                        foreach ($pivotUsers as $pU) {
                            if ($pU->hasRole('Delivery Guy')) {
                                $delivery_collection = DeliveryCollection::where('user_id', $pU->id)->first();
                                $cash_limit = $pU->delivery_guy_detail->cash_limit;
                                $inhand_cash = $delivery_collection ? $delivery_collection->amount : 0;

                                $is_in_limit = $cash_limit && $inhand_cash && $inhand_cash < $cash_limit;

                                if ($pU->delivery_guy_detail->status && $is_in_limit) {
    ...
    public function markSelfPickupOrderAsCompleted($id)
    {
        ...
        /*FoodomaProStoreDashBoardPro*/ 
        if (\Module::find("StoreDashBoardPro") && \Module::find("StoreDashBoardPro")->isEnabled()) { 
            $checkStatus = $order->orderstatus_id == '4' || $order->orderstatus_id == '7';
        } else {
            $checkStatus = $order->orderstatus_id == '7';
        }
        ...
        /*endFoodomaProStoreDashBoardPro*/
            ...
            if (\Module::find("Cashback") && \Module::find("Cashback")->isEnabled()) { 
                \Modules\Cashback\Traits\OrderTrait::payCashBack($order); 
            } 
            ...
            if ($order->payment_mode == 'COD' || $order->payment_mode == 'Cartão'  || $order->payment_mode == 'Pix'  ||  $order->payment_mode == 'Ticket Refeição' ||  $order->payment_mode == 'Caixa TEM' ||  $order->payment_mode == 'PicPay') {
                $restaurant_earning = RestaurantEarning::where('restaurant_id', $order->restaurant->id)
                    ->where('is_requested', 0)
                    ->first();
                if ($restaurant_earning) {
            ...
            if (config('appSettings.sendOrderInvoiceOverEmail') == 'true') {
                Mail::send('emails.invoice', ['order' => $order], function ($email) use ($order) {
                    $email->subject(config('appSettings.orderInvoiceEmailSubject') . '#' . $order->unique_order_id);
                    $email->from(config('appSettings.sendEmailFromEmailAddress'), config('appSettings.sendEmailFromEmailName'));
    ...
    public function saveNewRestaurant(Request $request)
    {
        ...
        if(\Module::find("Cashback") && \Module::find("Cashback")->isEnabled()) {
            ...
            if(!empty($cashbacksetting) && $cashbacksetting->restaurant_edit == 1){
                if ($request->cashback_status == 'true') { 
                    $restaurant->cashback_status = true; 
                } 
                else { 
                    $restaurant->cashback_status = false; 
                } 
                if(isset($request->cashback_value) && !empty($request->cashback_value)) { 
                    if (strpos($request->cashback_value, '%') !== false) { 
                        $restaurant->cashback_percentage = $request->cashback_value; 
                        $restaurant->cashback_value = '0.00'; 
                    }else{ 
                        $restaurant->cashback_value = $request->cashback_value; 
                        $restaurant->cashback_percentage = null; 
                    } 
                } 
                if(isset($request->cashback_limit_value) && !empty($request->cashback_limit_value)) { 
                    $restaurant->cashback_limit_value = $request->cashback_limit_value; 
                } 
            } 
        }
        /*endFoodomaProCashback*/
    ...
    public function updateRestaurant(Request $request)
    {
        ...
        if ($restaurant) {
            ...
            /* MEUAPP Permissão Lojista */
            $restaurant->delivery_charges = $request->delivery_charges;
            /* MEUAPP Permissão Lojista END */
            ...
            if (\Module::find("cashback") && \Module::find("cashback")->isEnabled()) { 
                ...
                if(isset($request->cashback_value) && !empty($request->cashback_value)) { 
                    if (strpos($request->cashback_value, '%') !== false) { 
                        $restaurant->cashback_percentage = (string)$request->cashback_value; 
                        $restaurant->cashback_value = "0.00"; 
                    }else { 
                        $restaurant->cashback_value = $request->cashback_value; 
                        $restaurant->cashback_percentage = null; 
            ...
            //WhatsApp for Store START
            
            $restaurant->storewhatsappApiKey = $request->storewhatsappApiKey;
            $restaurant->storewhatsAppNumber = $request->storewhatsAppNumber;
            
            
            //WhatsApp for Store END
    ...
    public function saveNewAddonCategory(Request $request)
    {
        ...
        if (\Module::find("AddonsPro") && \Module::find("AddonsPro")->isEnabled()) { 
            include(base_path('Modules/AddonsPro/includes/code/updateAddonCategory.php'));
        } 
        ...
        if ($order && $user) {
            if ($order->orderstatus_id == '1') {
                ...
                activity()
                    ->performedOn($order)
                    ->causedBy($user)
                    ->withProperties(['type' => 'cancel'])->log('Order cancelled by Store Owner');
    ...
    public function ratings($restaurant_id = null)
    {
        ...
        if ($restaurant_id) {
            ...
            $restaurant = Restaurant::where('id', $restaurant_id)->with(array('ratings' => function ($query) {
