...
class OrderController extends Controller
{
    ...
    public function placeOrder(Request $request, TranslationHelper $translationHelper)
    {
        ...
        if ($user) {
            ...
            if ($request['method'] == 'COD' || $request['method'] == 'Cartão' || $request['method'] == 'Pix' || $request['method'] == 'Ticket Refeição' || $request['method'] == 'Caixa TEM' || $request['method'] == 'PicPay') {
            ...
            if ($request['method'] == 'PAYPAL' || $request['method'] == 'PAYSTACK' || $request['method'] == 'RAZORPAY' || $request['method'] == 'STRIPE' || $request['method'] == 'PAYMONGO' || $request['method'] == 'MERCADOPAGO' || $request['method'] == 'PAYTM' || $request['method'] == 'FLUTTERWAVE' || $request['method'] == 'KHALTI') {
                ...
                if (\Module::find("NotifyStoreAndDeliveryGuy") && \Module::find("NotifyStoreAndDeliveryGuy")->isEnabled()) { 
                    // Send OneSignal Push Store Owner  - Changed by Josimar-Brasil
                    $restaurant_id = $request['order'][0]['restaurant_id'];
                    \Modules\NotifyStoreAndDeliveryGuy\Traits\PushNotifyTrait::oneSignalNotification('Store Owner',$restaurant_id);
                } 
                ...
                if ($restaurant->auto_acceptable && config('appSettings.enablePushNotification') && config('appSettings.enablePushNotificationOrders') == 'true') {
                    ...
                    /*FoodomaProNotifyStoreAndDeliveryGuy*/ 
                    if (\Module::find("NotifyStoreAndDeliveryGuy") && \Module::find("NotifyStoreAndDeliveryGuy")->isEnabled()) { 
                        \Modules\NotifyStoreAndDeliveryGuy\Traits\PushNotifyTrait::oneSignalNotification('Delivery Guy',$restaurant_id);
                    } 
                    /*endFoodomaProNotifyStoreAndDeliveryGuy*/
                    ...
                    foreach ($pivotUsers as $pU) {
                        ...
                        if ($pU->delivery_guy_detail->status && $is_in_limit) {
                            if ($newOrder->delivery_type == 1 && $newOrder->orderstatus_id != '8') {
                ...
                if (in_array($newOrder->orderstatus_id, ['1', '10']) && config('appSettings.oneSignalAppId') != null && config('appSettings.oneSignalRestApiKey') != null) {
            ...
            //if new payment gateway is added, write elseif here
             ...
             /*    
                ...
                *Address* - #".$newOrder->address = $full_address." 
   ...
   if (config('appSettings.enableStoreWhatsappNotification') == 'true') {
    ...
    $waStoreMsg = config('appSettings.whatsappStore');
$msgMessage = " *$waStoreMsg*
*Loja* -". $restaurant->name."
*Cliente Nº* - ". $user->phone."
*Endereço* - #".$newOrder->address = $full_address." 
*ID do Pedido* - #". $newOrder->unique_order_id."
*Valor de Pedido* -". config('appSettings.currencyFormat') .$newOrder->total;

    $message= urlencode($msgMessage);
    $storephone = $restaurant->storewhatsAppNumber;
    $storeapikey = $restaurant->storewhatsappApiKey;

    $url = 'https://api.callmebot.com/whatsapp.php?phone='.$storephone.'&text='.$message.'&apikey='.$storeapikey;
/*    $url = 'https://api.callmebot.com/whatsapp.php?phone='.$phone.'&text='.$message.'&apikey='.$apikey';*/
    $contents = file_get_contents($url);

}
//WhatsApp for STORE  END  
//WhatsApp for Admin END 
//WhatsApp Notification START
    ...
    public function cancelOrder(Request $request, TranslationHelper $translationHelper)
    {
        ...
        if ($order->user_id == $user->id && ($order->orderstatus_id == 1 || $order->orderstatus_id == 10)) {
            ...
            if ($order->payment_mode == 'COD' || $request['method'] == 'Cartão' || $request['method'] == 'Pix' || $request['method'] == 'Ticket Refeição' || $request['method'] == 'CAIXA TEM') {
            ...
            activity()
                ->performedOn($order)
                ->causedBy($user)
                ->withProperties(['type' => 'cancel'])->log('Order cancelled by customer');
    ...
    private function pushOneSignalToRestaurant($restaurant_id, $orderTotal)
    {
        //get restaurant
        $restaurant = Restaurant::where('id', $restaurant_id)->first();
        if ($restaurant) {
                ...
                foreach ($pivotUsers as $pU) {
                    if ($pU->hasRole('Store Owner')) {
                        ...
                        $message = $message;
                        $notify = new PushNotify();
                        $notify->oneSignalNotification('TO_RESTAURANT',$pU->email);
